#!/usr/bin/env node
/**
 * scripts/gen-egern.js v3.0.0
 * ───────────────────────────────────────────────
 * Cyberpunk Futuristic Egern Config Generator
 *
 * Features:
 *  - Multi-protocol support (SS, SSR, VLESS, VMess, Trojan, TUIC, Hysteria, WG)
 *  - Proxy groups: Proxy, Auto, Fallback, LoadBalance
 *  - Hardened DNS: Cloudflare + Google + AliDNS + DoH/DoT
 *  - CI/CD friendly metadata & SHA256 fingerprint
...
const ENV_OUTDIR = process.env.EGERN_OUTDIR;
const ENV_OUTPUT = process.env.EGERN_OUT;
if (CLI_OUTDIR && (ENV_OUTDIR || ENV_OUTPUT)) {
  console.warn("[WARN] CLI --outdir overrides EGERN_OUT/EGERN_OUTDIR environment variables.");
}
const DEFAULT_FILENAME = "egern.yaml";
const rawOutput = (() => {
...
function buildDNS() {
  return {
    ...
    nameserver: CLOUDFLARE_GOOGLE_DNS,
    fallback: [
...
function main() {
  ...
  const header = `# ✦ Neon Egern Config ✦
# Generated: ${new Date().toISOString()}
# Commit: ${gitCommit()}
# Generator: gen-egern.js v${VERSION}
...
`;
  ...
  if (DRY_RUN) console.log(outContent);
    ...
    console.log(`⚡ egern.yaml → ${OUTPUT} (${meta.hash.slice(0, 12)})`);
  }

  if (CI_MODE) console.log("CI-SUMMARY-JSON:", JSON.stringify(meta));
}

if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith("gen-egern.js")) main();
