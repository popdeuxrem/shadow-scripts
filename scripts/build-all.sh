...
if [[ ${#JS_LIST[@]} -eq 0 ]]; then
  ...
  | xargs -P "$(nproc)" -I{} bash -c '
      ...
      # output path was missing previously → fix with --output
      $OBF "$JS" \
        --output "$obf" \
        --compact true \
        --self-defending true \
      ...
      [[ -s "$obf" ]] || { echo "⚠️  Obfuscation failed: $obf not created" >&2; exit 1; }
      base64 "$obf" > "$b64"
      [[ -s "$b64" ]] || { echo "⚠️  Empty .b64 payload: $b64" >&2; rm -f "$b64"; exit 1; }
    '
  rm -rf "$TMPDIR"
fi
shopt -u globstar nullglob

# ─── 4 · manifest & loader assets ─────────────────────────────────────────────
